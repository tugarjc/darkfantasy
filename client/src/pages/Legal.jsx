    ...
    <div className="min-h-screen bg-deep text-parchment">
      <header className="bg-base border-b border-border px-4 py-3 flex items-center justify-between">
        ...
        <nav className="flex gap-4 text-sm">
          ...
          <Link to="/legal" className="text-gold">{t('legal.legal_link')}</Link>
        </nav>
      </header>

      <main className="max-w-3xl mx-auto px-4 py-12">
        <h1 className="font-display text-3xl text-gold mb-8">{t('legal.mentions_title')}</h1>

        <Section title={t('legal.mentions_publisher')}>
          <p>{t('legal.mentions_publisher_text')}</p>
        </Section>

        <Section title={t('legal.mentions_hosting')}>
          <p>{t('legal.mentions_hosting_text')}</p>
        </Section>

        <Section title={t('legal.mentions_contact')}>
          <p>{t('legal.mentions_contact_text')}</p>
        </Section>
